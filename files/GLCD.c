...
void LCD_draw_circle_full( int x0, int y0, int radius, uint16_t color) 
{
  ...
  while (x >= y) 
  {
    ...
    LCD_SetPoint(-x + x0, y + y0, color);
    LCD_SetPoint(-y + x0, x + y0, color);
		LCD_draw_line(-x + x0, y + y0, x + x0, y + y0, color);
		LCD_draw_line(-y + x0, x + y0, y + x0, x + y0, color);
    LCD_SetPoint(-x + x0, -y + y0, color);
    LCD_SetPoint(-y + x0, -x + y0, color);
    LCD_SetPoint(x + x0, -y + y0, color);
    LCD_SetPoint(y + x0, -x + y0, color);
...
void LCD_draw_block(int x, int y, int w, int h, uint16_t color) {
	int i,j;
...
void LCD_draw_smiley_face( int x0, int y0, int radius, uint16_t color)
{
	...
	LCD_draw_semicircle(x0, y0+(radius/4), radius/2, color);
	LCD_draw_circle_full(x0-(radius/2), y0-(radius/4), radius/8, color);
	LCD_draw_circle_full(x0+(radius/2), y0-(radius/4), radius/8, color);
}

void LCD_draw_snake( int nX, int nY, int oX, int oY ) {
	LCD_draw_block(nX, nY, 23, 23, Green);
	if(oX >= 0)
		LCD_draw_block(oX, oY, 23, 23, White);
}

void LCD_draw_mouse( int x, int y ) {
	LCD_draw_block(x, y, 23, 23, Purple);
}

void LCD_draw_ufo( int x0, int y0, int state) {
	int i;
	uint16_t ctable[] = { Red, Yellow };
	
	//Top two blue blocks
	LCD_draw_block(x0, y0 + 24, 8, 56, Blue);
	LCD_draw_block(x0 - 8, y0 + 16, 8, 72, Blue);
	
	//alternating red/yellow lights
	for(i = 0; i < 13; i++) {
		if(state)
			LCD_draw_block(x0 - 16, y0 + (i * 8), 8, 8, ctable[i%2]);
		else
			LCD_draw_block(x0 - 16, y0 + (i * 8), 8, 8, ctable[!(i%2)]);
	}
	
	//Bottom two blue blocks
	LCD_draw_block(x0 - 24, y0 + 16, 8, 72, Blue);
	LCD_draw_block(x0 - 32, y0 + 24, 8, 56, Blue);
}

void LCD_draw_grenade( int x0, int y0 ) {
	LCD_draw_block(x0, y0, 24, 16, Magenta);
}

void LCD_clear_ufo( int x0, int y0, int state) {
